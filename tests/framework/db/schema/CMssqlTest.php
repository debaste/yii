...
EOD;
	...
	public function testColumn()
	{
		$values=[
			...
			'size'=>[10, 128, null, 10, null],
			'precision'=>[10, 128, null, 10, null],
			'scale'=>[0, null, null, 0, null],
			'dbType'=>['int','varchar','datetime','int','text'],
		...
		$values=[
			...
			'defaultValue'=>[null, 1, null, "something", null, null, '1.23', null, '33.22', '2002-01-01 00:00:00', null, true],
			'size'=>[10, 10, 100, 100, null, 24, 53, null, 5, null, null, null],
			'precision'=>[10, 10, 100, 100, null, 24, 53, null, 5, null, null, null],
			'scale'=>[0, 0, null, null, null, null, null, null, 2, null, null, null],
			'dbType'=>['int','int','char','varchar','text','real','float','image','numeric','datetime','bit','bit'],
			'type'=>['integer','integer','string','string','string','double','double','string','string','string','boolean','boolean'],
	...
	public function testTransactions()
	{
		...
		try
		{
			$builder->createInsertCommand($table, ['title'=>'working transaction test post 1','create_time'=>'2009-01-01','author_id'=>1,'content'=>'test content'])->execute();
			$builder->createInsertCommand($table, ['title'=>'working transaction test post 2','create_time'=>'2009-01-01','author_id'=>1,'content'=>'test content'])->execute();
			$transaction->commit();
		}
		catch (Exception $e)
		{
		...
		$n=$builder->createCountCommand($table, new CDbCriteria(['condition' => "title LIKE 'working transaction%'"]))->queryScalar();
		...
		try
		{
			$builder->createInsertCommand($table, ['title'=>'failed transaction test post 1','create_time'=>'2009-01-01','author_id'=>1,'content'=>'test content'])->execute();
			$builder->createInsertCommand($table, ['id' => 1, 'title'=>'failed transaction test post 2','create_time'=>'2009-01-01','author_id'=>1,'content'=>'test content'])->execute();
			$transaction->commit();
		}
		catch (Exception $e)
		{
		...
		$n=$builder->createCountCommand($table, new CDbCriteria(['condition' => "title LIKE 'failed transaction%'"]))->queryScalar();
	...
	public function testResetSequence()
	{
		...
		$this->db->createCommand()->insert('users',['username'=>'testerX','password'=>'passwordX','email'=>'emailX@gmail.com']);
		...
		$this->db->createCommand()->insert('users',['username'=>'testerY','password'=>'passwordY','email'=>'emailY@gmail.com']);
		$id=$this->db->createCommand()->select('id')->from('users')->where("[username]='testerY'")->queryScalar();
		$this->assertEquals(100,$id);

		$this->db->schema->resetSequence($tables['users']);
		$this->db->createCommand()->insert('users',['username'=>'testerZ','password'=>'passwordZ','email'=>'emailZ@gmail.com']);
