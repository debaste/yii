...
class CActiveRecord2Test extends CTestCase
{
	...
	public function testModel()
	{
		...
		$this->assertEquals([],$model->getAttributes(false));
	...
	public function testFind()
	{
		...
		$post=Post2::model()->find('id=:id',[':id'=>2]);
		$this->assertTrue($post instanceof Post2);
		$this->assertEquals(2,$post->id);

		$post=Post2::model()->find(['condition'=>'id=:id','params'=>[':id'=>3]]);
		$this->assertTrue($post instanceof Post2);
		...
		$posts=Post2::model()->findAll(new CDbCriteria(['limit'=>3,'offset'=>1]));
		...
		$post=Post2::model()->findByPk([3,2]);
		$this->assertEquals(2,$post->id);

		$post=Post2::model()->findByPk([]);
		$this->assertNull($post);
		...
		$posts=Post2::model()->findAllByPk([4,3,2],'id<4');
		$this->assertEquals(2,count($posts));
		$this->assertEquals(2,$posts[0]->id);
		$this->assertEquals(3,$posts[1]->id);

		$posts=Post2::model()->findAllByPk([]);
		$this->assertTrue($posts===[]);

		// test findByAttributes
		$post=Post2::model()->findByAttributes(['author_id'=>2],['order'=>'id DESC']);
		$this->assertEquals(4,$post->id);

		// test findAllByAttributes
		$posts=Post2::model()->findAllByAttributes(['author_id'=>2]);
		$this->assertEquals(3,count($posts));

		// test findBySql
		$post=Post2::model()->findBySql('select * from test.posts where id=:id',[':id'=>2]);
		$this->assertEquals(2,$post->id);

		// test findAllBySql
		$posts=Post2::model()->findAllBySql('select * from test.posts where id>:id',[':id'=>2]);
		$this->assertEquals(3,count($posts));

		// test count
		$this->assertEquals(5,Post2::model()->count());
		$this->assertEquals(3,Post2::model()->count(['condition'=>'id>2']));

		// test countBySql
		$this->assertEquals(1,Post2::model()->countBySql('select id from test.posts limit 1'));

		// test exists
		$this->assertTrue(Post2::model()->exists('id=:id',[':id'=>1]));
		$this->assertFalse(Post2::model()->exists('id=:id',[':id'=>6]));
	}

	public function testInsert()
	{
		$post=new Post2;
		$this->assertEquals([],$post->getAttributes(false));
	...
	public function testUpdate()
	{
		...
		// test updateByPk
		$this->assertEquals(2,Post2::model()->updateByPk([4,5],['title'=>'test post']));
		...
		// test updateAll
		$this->assertEquals(1,Post2::model()->updateAll(['title'=>'test post'],'id=1'));
		...
		$this->assertEquals(3,Post2::model()->updateCounters(['author_id'=>-1],'id>2'));
		$this->assertEquals(2,Post2::model()->findByPk(2)->author_id);
	...
	public function testDelete()
	{
		...
		$this->assertEquals(2,Post2::model()->deleteByPk([2,3]));
	...
	public function testCompositeKey()
	{
		...
		$order=Order2::model()->findByPk(['key1'=>2,'key2'=>1]);
		$this->assertEquals('order 21',$order->name);
		$orders=Order2::model()->findAllByPk([['key1'=>2,'key2'=>1],['key1'=>1,'key2'=>3]]);
	...
	{
		$post=new PostExt2;
		$this->assertEquals(['id'=>null,'title'=>'default title'],$post->getAttributes(false));
		$post=Post2::model()->findByPk(1);
		$this->assertEquals([
			'id'=>1,
			'title'=>'post 1',
	...
	public function testLazyRelation()
	{
		...
		// test composite key
		$order=Order2::model()->findByPk(['key1'=>1,'key2'=>2]);
	...
	{
		$post=Post2::model()->with(['comments'=>'author','categories'])->findByPk(2);
		$this->assertEquals(2,count($post->comments));
		$this->assertEquals(2,count($post->categories));
	}
	...
	{
		$posts=Post2::model()->with('comments')->findAllByPk([2,3,4],['order'=>'t.id']);
		$this->assertEquals(3,count($posts));
		...
		$post=Post2::model()->with('comments')->findByAttributes(['id'=>2]);
		$this->assertTrue($post instanceof Post2);
		$this->assertEquals(2,count($post->comments));
		$posts=Post2::model()->with('comments')->findAllByAttributes(['id'=>2]);
		$this->assertEquals(1,count($posts));

		$post=Post2::model()->with('comments')->findBySql('select * from test.posts where id=:id',[':id'=>2]);
		$this->assertTrue($post instanceof Post2);
		$posts=Post2::model()->with('comments')->findAllBySql('select * from test.posts where id=:id1 OR id=:id2',[':id1'=>2,':id2'=>3]);
		$this->assertEquals(2,count($posts));

		$post=Post2::model()->with('comments','author')->find('t.id=:id',[':id'=>2]);
		$this->assertTrue($post instanceof Post2);

		$posts=Post2::model()->with('comments','author')->findAll([
			'select'=>'title',
			'condition'=>'t.id=:id',
			'limit'=>1,
			'offset'=>0,
